...
import (
	...
	"github.com/rakim0/femProject/internal/tokens"
...
func (h *TokenHandler) HandleCreateToken(w http.ResponseWriter, r *http.Request) {
	...
	tokens, err := h.tokenStore.CreateNewToken(user.ID, 24*time.Hour, tokens.ScopeAuth)
	if err != nil {
		h.logger.Printf("ERROR: Could not create token %v", err)
		utils.WriteJSON(w, http.StatusInternalServerError, utils.Envelope{"error": "internal server error"})
		return
	}
	utils.WriteJSON(w, http.StatusCreated, utils.Envelope{"auth_token": tokens})
}
